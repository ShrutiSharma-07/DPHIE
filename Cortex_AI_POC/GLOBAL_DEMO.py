...
CORTEX_SEARCH_SCHEMA = "POC_CHUNKS"
CORTEX_SEARCH_SERVICE = "POC_CORTEX_SEARCH_SERVICE"
...
def initialize_chat_history_table():
    ...
    try:
        ...
        create_table_sql = """
        CREATE TABLE IF NOT EXISTS CORTEX_POC.POC_CHUNKS.CHAT_HISTORY (
        ...
        for column_name in columns_to_check:
            try:
                ...
                if len(result) == 0:
                    ...
                    alter_table_sql = f"""
                    ALTER TABLE CORTEX_POC.POC_CHUNKS.CHAT_HISTORY 
...
def create_source_document_links(source_docs):
    ...
    for path in source_docs:
        try:
            cmd = f"select GET_PRESIGNED_URL(@poc_documents, '{path}', 360) as URL_LINK from directory(@poc_documents)"
...
def update_feedback(interaction_id, feedback):
    ...
    try:
        ...
        update_sql = f"""
        UPDATE CORTEX_POC.POC_CHUNKS.CHAT_HISTORY 
        ...
        FROM CORTEX_POC.POC_CHUNKS.CHAT_HISTORY 
        WHERE INTERACTION_ID = '{interaction_id}'
        """
...
def store_chat_interaction(question, response, model_name, category, source_docs, response_time_ms):
    ...
    try:
        ...
        if review_column_exists:
            ...
            insert_sql = """
            INSERT INTO CORTEX_POC.POC_CHUNKS.CHAT_HISTORY 
...
def get_document_links(relative_paths):
    """Get document links from DOCUMENT_LINKS table for given relative paths"""
    try:
        ...
        SELECT DISTINCT 
            ...
            B.DOCUMENT_NAME, 
            B.LINK 
        FROM POC_DOCS_CHUNKS A 
        INNER JOIN DOCUMENT_LINKS B 
...
def show_context_documentation():
    ...
    for path in st.session_state.last_relative_paths:
        ...
        if path in available_links:
            ...
            try:
                cmd2 = f"select GET_PRESIGNED_URL(@poc_documents, '{path}', 360) as URL_LINK from directory(@poc_documents)"
...
def config_options():
    ...
    categories = session.table('POC_DOCS_CHUNKS').select('category').distinct().collect()
...
def create_prompt(myquestion):
    ...
    prompt = f"""
           ...
           provide a good amount of detail and do not hallucinate. 
...
def answer_question(myquestion):
    ...
    # Add guardrails option to Complete function
    response = Complete(
...
def main():
    ...
    st.title(f"Document Chat Assistant")
    
    st.write("List of documents provided in context")
    docs_available = session.sql("ls @poc_documents").collect()
    list_docs = []
    ...
    # Accept user input
    if question := st.chat_input("What do you want to know from the documents?"):
